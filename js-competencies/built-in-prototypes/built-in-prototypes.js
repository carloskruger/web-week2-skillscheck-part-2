...
function dragonMapper(arr){
  let newArr = arr.map(elem => elem += '...here be dragons')
  return newArr
}
...
function cookieLoversOnly(arr){
  let filteredArr = arr.filter(function(elem) { if(elem['favoriteCookie']) return elem })
return filteredArr
}
